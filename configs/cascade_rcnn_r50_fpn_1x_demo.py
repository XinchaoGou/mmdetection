...
data_root = '../data/coco/'  # 这里的路径是相对与train的路径
img_norm_cfg = dict(
...
data = dict(
    imgs_per_gpu=8,  #每张gpu训练多少张图片  batch_size = gpu_num(训练使用gpu数量) * imgs_per_gpu
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
...
# optimizer
optimizer = dict(type='SGD', lr=0.04, momentum=0.9, weight_decay=0.0001)  #学习率的设置尤为关键：lr = 0.00125*batch_size
...
load_from = '../checkpoints/cascade_rcnn_r50_coco_pretrained_weights_classes_21.pth' #采用coco预训练模型 ,需要对权重类别数进行处理
resume_from = None
workflow = [('train', 1)]